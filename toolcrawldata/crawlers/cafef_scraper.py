    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        for a_tag in soup.find_all("a", href=re.compile(r'.*-\d{10,}\.chn$')):
            ...
            articles.append({
                ...
                "source":       "cafef",
                "article_url":  href,
                "published_at": pub_dt,  # None nếu không parse được, sẽ enrich từ trang bài viết
...
def fetch_content(idx: int, article: dict) -> tuple[int, dict]:
    ...
    try:
        ...
        # Luôn parse ngày từ trang bài viết để đảm bảo chính xác
        for sel in ["meta[property='article:published_time']", "meta[name='pubdate']",
                    "time[datetime]", "span.pdate", "span.time", ".entry-date", ".post-date"]:
